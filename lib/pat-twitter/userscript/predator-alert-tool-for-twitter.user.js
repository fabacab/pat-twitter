...
// @version        0.1
// @namespace      com.maybemaimed.pat.twitter
...
// @require        lib/pat-twitter/userscript/oauth-1.0a/oauth-1.0a.js
...
PATTwitter.List.export = function () {
    var url = 'data:application/json;charset=utf-8,' + encodeURIComponent(PATTwitter.getValue('list_' + this.local_key));
    GM_openInTab(url);
};
...
PATTwitter.UI.Dashboard.init = function () {
    ...
    //     1. Inject HTML templates so we have the needed markup.
    //     2. Optionally, define and attach Flight JS components.
        ...
        (function (i) {
            ...
            var listMembershipComponent = flight.component(function () {
                ...
                this.onClick = function (e) {
                    ...
                    var twit = new PATTwitter.Predator(twitter_id);
                    twit.screen_name = screen_name;
                    // Show another pop up asking for a reason this user is added to said list.
                    // This user input becomes the "alert description."
                    // TODO: Offer a nicer interface than this.
                    var alert_desc = window.prompt('You can choose to include a reason why you are adding @' + screen_name + ' to this list.');
                    twit.addAlert({'alert_list': this.$node.data('list-name'), 'alert_desc': alert_desc});
                    if (twit.save()) {
                        ...
                        this.$node.unbind();
                        this.$node.on('click', this.captureClick);
                        PATTwitter.refreshRedboxes();
                    }
                };
