...
PATTwitter.UI.Templates.support_circle_link = '<li role="presentation"><a href="#pat-twitter-support-circle-dialog">My Support Circle</a></li>';
        ...
        <div class="modal-dialog">\
            <div class="modal-content">\
                ...
                <div class="modal-body">\
                    ...
                    <ul>\
                        <li><a href="https://github.com/fabacab/pat-twitter/wiki/User-Manual">Read the friendly user manual</a></li>\
                        <li><a href="https://github.com/fabacab/pat-twitter/wiki/Frequently-Asked-Questions">Frequently Asked Questions</a></li>\
                    </ul>\
...
PATTwitter.UI.Templates.tweet_action_support_circle = '\
<li class="pat-twitter-support-circle-link js-AlertSupportCircle" role="presentation">\
    <button class="dropdown-link" type="button" role="menuitem">Get help from your Support Circle</button>\
</li>';
PATTwitter.UI.Templates.support_circle_dialog = '\
<div class="pat-twitter-modal-container modal-container" id="pat-twitter-support-circle-dialog">\
    <div class="close-modal-background-target"></div>\
    <div class="modal modal-medium draggable" id="pat-twitter-support-circle-dialog-dialog" role="alertdialog" aria-labelledby="pat-twitter-suport-circle-dialog-header" aria-describedby="pat-twitter-suport-circle-dialog-body"><div tabindex="0" class="js-first-tabstop"></div>\
        <div class="modal-content" role="document">\
            <div class="modal-header">\
                <h3 class="modal-title" id="pat-twitter-suport-circle-dialog-header">My Support Circle</h3>\
            </div>\
            <div class="modal-body" id="pat-twitter-suport-circle-dialog-body">\
                <div id="pat-twitter-suport-circle-menu" class="pat-twitter-suport-circles clearfix">\
                    <div class="list-editor">\
                        <p>Enter the Twitter names of friends and supporters who can back you up if you get harassed or bullied on Twitter.</p>\
                        <ol>\
                            <li><div class="field">@<input name="pat-twitter-support-circle-supporter[]" /></div></li>\
                            <li><div class="field">@<input name="pat-twitter-support-circle-supporter[]" /></div></li>\
                            <li><div class="field">@<input name="pat-twitter-support-circle-supporter[]" /></div></li>\
                            <li><div class="field">@<input name="pat-twitter-support-circle-supporter[]" /></div></li>\
                            <li><div class="field">@<input name="pat-twitter-support-circle-supporter[]" /></div></li>\
                    ...
                    <div class="list-editor-save">\
                        <button class="btn btn-primary" type="button">Save Support Circle</button>\
                    </div>\
                </div>\
            </div>\
        </div>\
    <div tabindex="0" class="js-last-tabstop"></div></div>\
</div>';
...
PATTwitter.UI.ListsPage.init = function () {
    ...
    el.find('.ListCreationModule-description').append('<a href="https://github.com/fabacab/pat-twitter/wiki/User-Manual:Using-PAT-Twitter-Lists" class="ListCreationModule-moreInfoLink">More info</a>');
...
PATTwitter.UI.init = function () {
    ...
    if (jQuery('[data-name="lists"]').length) {
        PATTwitter.UI.injectTemplate('home_server_link', {}, jQuery('[data-name="lists"]'), 'insertAfter');
        var x = PATTwitter.UI.injectTemplate('support_circle_dialog', {}, document.body);
        var supporters = PATTwitter.getValue('my_support_circle', '').split(',');
        var els = x.find('input[name^="pat-twitter-support-circle-supporter"]');
        for (var i = 0; i < supporters.length; i++) {
            jQuery(els).eq(i).val(supporters[i]);
        }
        var x = PATTwitter.UI.injectTemplate('support_circle_link', {}, jQuery('[data-name="lists"]'), 'insertAfter');
        x.on('click', function () {
            jQuery('#pat-twitter-support-circle-dialog').modal();
        });

        jQuery('#pat-twitter-support-circle-dialog .btn-primary').on('click', PATTwitter.saveSupportCircle);
    }
    ...
    // Modify any tweet UI elements to add our own buttons, etc.
    jQuery('.tweet, [data-item-type="tweet"]').each(PATTwitter.attachSupportCircleLink);
...
//                        PATTwitter.log(mutation.addedNodes[i]);
                        ...
                        // If the new node is a tweet...
                        if (jQuery(mutation.addedNodes[i]).is('.tweet, [data-item-type="tweet"]')) {
                            PATTwitter.attachSupportCircleLink.call(mutation.addedNodes[i]); // use node as `this` value
                        } else if (jQuery(mutation.addedNodes[i]).find('.tweet, [data-item-type="tweet"]').length) { // ...or has tweet nodes in it
                            jQuery(mutation.addedNodes[i]).find('.tweet, [data-item-type="tweet"]').each(PATTwitter.attachSupportCircleLink);
                        }
                    }
                }
            }
...
// Helpers.
//
PATTwitter.saveSupportCircle = function (e) {
    var supporters = [];
    var dialog = jQuery(e.target).parents('.modal-container');
    dialog.find('input[name^="pat-twitter-support-circle-supporter"]').each(function () {
        supporters.push(jQuery(this).val());
    });
    PATTwitter.setValue('my_support_circle', supporters.join(','));
    dialog.modal('toggle');
};
PATTwitter.attachSupportCircleLink = function () {
    if (0 === jQuery(this).find('.pat-twitter-support-circle-link').length) {
        var x = PATTwitter.UI.injectTemplate(
            'tweet_action_support_circle',
            {},
            jQuery(this).find('.more-tweet-actions ul, .js-more-tweet-actions ul')
        );
        x.on('click', PATTwitter.callForSupportCircleHelp);
    }
};
PATTwitter.callForSupportCircleHelp = function () {
    PATTwitter.log('Calling for help from Support Circle!');
    var tweet = jQuery(this).parents('.tweet, [data-item-type="tweet"]');
    if (tweet.length) { tweet = tweet.eq(0); }
    var msg = 'I\'m getting bullied on Twitter by @' + tweet.data('screen-name') + ': https://twitter.com/' + tweet.data('screen-name') + '/status/' + tweet.data('tweet-id') + ' Please help?';
    jQuery('.global-dm-nav').click();
    var supporters = PATTwitter.getValue('my_support_circle').split(',');
    for (var i = 0; i < supporters.length; i++) {
        if (supporters[i].length) { // don't DM empty usernames
            PATTwitter.log('Sending call for help to ' + supporters[i]);
            jQuery('.dm-new-button').click();
            jQuery('.dm-to-input').val(supporters[i]);
            jQuery('#tweet-box-dm-new-conversation').text(msg);
            jQuery('#tweet-box-dm-new-conversation').focus();
            jQuery('#dm_dialog_new .tweet-btn').prop('disabled', false);
            jQuery('#dm_dialog_new .tweet-btn').removeClass('disabled');
            jQuery('#dm_dialog_new .tweet-btn').click();
        }
    }
    jQuery('#dm_dialog-dialog .js-close').click();
};
